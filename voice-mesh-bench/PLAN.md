...
8. Only then add the Godot audio-thread, source-count, spatial-mixing, and
   ...
   **Blocked on receiver tracing:** the first headless Godot 4.7.2 gate creates one
   ...
   3D even while packets and the main loop continue normally. Separate physical
   hosts reproduce the pauses with no cgroup throttling; callback entry/exit
   timers locate the delay outside our callback. Next capture Godot/PulseAudio
   waits; gna-sim currently denies PTRACE_TRACEME. See `GODOT_SCALE_RESULTS.md`
   and `GODOT_CHURN_RESULTS.md`.
